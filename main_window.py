...
class MainWindow:
    ...
    def __init__(self):  # instantiation function. Use root for GUI and refers to main window
        ...
        self.rows_to_display = [0] * self.user_entry.n_rows_to_peak
        ...
        # Frame-2 Session
        # Labels
        label_file_name = Label(self.frame_root_session, text="Filename")
        ...
        label_header_include = Label(self.frame_root_session, text="Include Header in Plot")
        ...
        # label_row_indices = Label(self.frame_root_session, text="Row Indices")
        ...
        label_data_end = Label(self.frame_root_session, text="Data End Index")
        ...
        self.entry_columns_to_process = Entry(self.frame_root_session, width=80, textvariable=self.entry_columns_to_process_entry,
                                        validate="key", validatecommand=(entry_validation_positive_numbers_comma, '%S'))
        self.entry_columns_to_process.insert(END, str(PROCESS_ALL_COLUMNS))
        self.entry_columns_to_process.configure(state='disabled')

        self.entry_header_row_entry = IntVar()
        self.entry_header_row_entry.trace("w", lambda name, index, mode, entry_header_row_entry=self.entry_header_row_entry: self.entry_update_header_row())
        self.entry_header_row = Entry(self.frame_root_session, width=10, textvariable=self.entry_header_row_entry, validate="key", validatecommand=(entry_validation_positive_numbers, '%P'))
        self.entry_header_row.insert(END, str(ROW_HEADER))

        self.entry_data_start_row_entry = IntVar()
        self.entry_data_start_row_entry.trace("w", lambda name, index, mode, entry_data_start_row_entry=self.entry_data_start_row_entry: self.entry_update_data_start_row())
        self.entry_data_start_row = Entry(self.frame_root_session, width=10, textvariable=self.entry_data_start_row_entry, validate="key", validatecommand=(entry_validation_positive_numbers, '%P'))
        self.entry_data_start_row.insert(END, str(ROW_DATA_START))

        self.entry_data_end_row_entry = IntVar()
        self.entry_data_end_row_entry.trace("w", lambda name, index, mode, entry_data_end_row_entry=self.entry_data_end_row_entry: self.entry_update_data_end_row())
        self.entry_data_end_row = Entry(self.frame_root_session, width=10, textvariable=self.entry_data_end_row_entry, validate="key", validatecommand=(entry_validation_positive_numbers, '%P'))
        self.entry_data_end_row.insert(END, str(ROW_DATA_END))

        # Radio buttons for file saving
        ...
        self.checkbox_timestamp_extract = Checkbutton(self.frame_root_session, variable=self.checkbox_timestamp_extract_entry, command=self.checkbox_update_timestamp_extract)

        self.checkbox_timestamp_include_entry = IntVar(value=self.user_entry.timestamp_include)
        self.checkbox_timestamp_include = Checkbutton(self.frame_root_session, variable=self.checkbox_timestamp_include_entry, command=self.checkbox_update_timestamp_include)

        self.checkbox_header_include_entry = IntVar(value=self.user_entry.header_include)
        self.checkbox_header_include = Checkbutton(self.frame_root_session,
                                                      variable=self.checkbox_header_include_entry,
                                                      command=self.checkbox_update_header_include)

        # Buttons
        self.button_choose_folder = Button(self.frame_root_session, text="Choose File", command=self.button_choose_file, pady=0, width=10)
        self.button_file_peak = Button(self.frame_root_session, text="File Peak", command=self.button_file_peak, pady=0, width=10, fg='brown')

        # Grids
        label_file_name.grid                        (row=5, column=0, sticky=E)
        self.entry_file_name.grid                   (row=5, column=1, sticky=W)
        self.button_choose_folder.grid              (row=6, column=0, sticky=E)
        ...
        # label_header_include.grid                   (row=9, column=1, sticky=W,  padx=(150, 0))
        # self.checkbox_header_include.grid           (row=9, column=1, sticky=W,  padx=(250, 0))
        ...
        self.entry_data_end_row.grid                (row=15, column=1, sticky=W)
        ...
        label_header_include.grid                   (row=16, column=1, sticky=W, padx=(300, 0))
        self.checkbox_header_include.grid           (row=16, column=1, sticky=W, padx=(425, 0))
        ...
        Hovertip(self.entry_data_end_row, tooltip_entry_data_end_row)
        ...
        self.button_plot_data = Button(self.frame_root_commands, text="Plot Data", pady=3, width=20, fg="green",
                                       command=self.start_plot_figures)
        self.button_load_file = Button(self.frame_root_commands, text="Load File", pady=3, width=20, fg="green",
    ...
    # def gui_load_file_button_disable(self):
    #     self.button_load_file["state"] = DISABLED

    # def gui_load_file_button_enable(self):
    #     self.button_load_file_recreate()

    # def button_load_file_recreate(self):
    #     self.button_load_file.destroy()
    #     self.button_load_file = Button(self.frame_root_commands, text="Load File", pady=3, width=20, fg="green",
    #                                       command=self.load_file)
    #     self.button_load_file.grid(row=1, column=3, padx=(0, 3))
    ...
    def entry_update_data_start_row(self):
        try:
            ...
            self.user_entry.data_start_row_index = ROW_DATA_START
            print("::user_entry.data_start_row_index: ", self.user_entry.data_start_row_index)

    def entry_update_data_end_row(self):
        try:
            entry_data_end_row = self.entry_data_end_row_entry.get()
            self.user_entry.data_end_row_index = int(entry_data_end_row)
            print("::user_entry.data_end_row_index: ", self.user_entry.data_end_row_index)
        except:
            self.user_entry.data_end_row_index = ROW_DATA_END
            print("::user_entry.data_end_row_index: ", self.user_entry.data_end_row_index)
    ...
    def entry_update_file_name_and_suffix(self):
        ...
        if fm.FileNameMethods.check_filename_components_exists(self.user_entry.file_location, file_name, file_suffix):
            ...
            # try:
            #     self.gui_load_file_button_enable()
    ...
    def load_defaults(self):
        ...
        self.entry_data_end_row.delete(0, END)
        ...
        self.entry_data_start_row.      insert(0, ROW_DATA_START)
        self.entry_data_end_row.        insert(0, ROW_DATA_END)
    ...
    def peak_file(self):
        ...
        message = 'File in Processing: ' + fm.FileNameMethods.build_file_name_full(
        ...
        self.session_process_csv = SessionProcessCSV(self.user_entry, self.session_log, self.textbox_rows, self.session_option)
    ...
    def start_save_csv(self):
        ...
        #     self.save_csv()
        # except:
        #     self.message_box('Save Error', 'Failed to save .csv files\n-Check if data file is loaded.'
        #                                    '\n-Check columns and file structure.')
    ...
    def save_csv(self):
        ...
        # if self.user_entry.header_include:
        #     pass
        # else:
        self._save_data_pandas_delimiter(self.user_entry.file_location, file_name,
                                             data_to_write, self.user_entry.write_delimiter_type)
    ...
    def _save_data_pandas_delimiter(self, folder, file_name, data, delimiter):
        ...
        if delimiter == DELIMITER_TYPE_COMMA:
            pd.DataFrame(data).to_csv(file_address, header=False, index=False, sep=',')
    ...
    def display_session_settings(self):
        ...
        message = 'Columns to Process: ' + str(self.user_entry.column_choice_list) + '\n'
        self.session_log.write_textbox(message, colour)

        message = 'Extract Timestamp: ' + ('yes' if self.user_entry.timestamp_extract == 1 else 'no') + '\n'
        self.session_log.write_textbox(message, colour)

        message = 'Read Delimiter: ' + ('Comma' if self.user_entry.read_delimiter_type == DELIMITER_TYPE_COMMA else 'Tab') + '\n'
        self.session_log.write_textbox(message, colour)

        message = ' Header at Row: ' + str(self.user_entry.header_row_index) + '\n'
        self.session_log.write_textbox(message, colour)

        # message = 'Units at Row: ' + str(self.user_entry.units_row) + '\n'
        # self.session_log.write_textbox(message, colour)

        message = 'Data Start at Row: ' + str(self.user_entry.data_start_row_index) + '\n'
        self.session_log.write_textbox(message, colour)

        message = 'Data End at Row: ' + str(self.user_entry.data_end_row_index) + '\n'
        self.session_log.write_textbox(message, colour)

        message = 'Include Header in Plot: ' + str(self.user_entry.header_include) + '\n'
        self.session_log.write_textbox(message, colour)
    ...
    def checkbox_update_header_include(self):
        checkbox_header_include_entry = self.checkbox_header_include_entry.get()
        self.user_entry.header_include = checkbox_header_include_entry
        print("::user_entry.header_include: ", self.user_entry.header_include)
